...
SnoCdnLoader::CdnImpl::CdnImpl(std::string const& build, std::string lang) {
  ...
  SnoCdnLoader::parsebuild(buildConfig, this->build);

  SnoManager::loadTOC(toc.data());
}

bool SnoCdnLoader::parsebuild(std::map<std::string, std::string> const& config, BuildInfo& info) {
  if (!config.count("build-name")) return false;
  std::string const& name = config.at("build-name");
  info.version.clear();
...
std::vector<std::string> SnoCdnLoader::builds() {
  ...
  result.push_back(ngdp.version().build);
  if (!ngdp.version().build2.empty()) {
    result.push_back(ngdp.version().build2);
  }
  return result;
}
...
std::map<istring, std::string> SnoCdnLoader::install() {
  ...
  if (!file || file.read16(true) != 0x494e /*IN*/) return result;
...
uint32 SnoCdnLoader::build() const {
  return handle_->build.build;
}
std::string SnoCdnLoader::version() const {
  return handle_->build.version;
}
