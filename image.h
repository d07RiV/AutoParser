...
public:
  ...
  {
    typename ImageBase<PF>::color_t const* src = image.bits();
    for (uint32 i = 0; i < data_->width_ * data_->height_; ++i) {
      data_->bits_[i] = Format::template from<PF>(src[i]);
    }
  }
  ...
  ImageBase<Format> subimage(int left, int top, int right, int bottom) const {
    if (left < 0) left = 0;
    if (top < 0) top = 0;
    if (right > static_cast<int>(data_->width_)) right = data_->width_;
    if (bottom > static_cast<int>(data_->height_)) bottom = data_->height_;
