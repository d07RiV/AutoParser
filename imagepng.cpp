...
#pragma	pack(pop)
        ...
        for (uint32 x = 0; x < width; x++) {
          ...
          if (hdr.colorType == 0) {
            ...
            if (pal.useColorKey && gray == pal.colorKey[0]) {
              ...
              cur = Image::Format::color(gray, gray, gray);
            }
            ...
            if (pal.useColorKey && red == pal.colorKey[0] &&
              ...
              cur = Image::Format::color(fix_color(red, hdr.bitDepth),
            ...
            cur = Image::Format::color(gray, gray, gray, alpha);
            ...
            uint32 alpha = fix_color(buf.read(hdr.bitDepth), hdr.bitDepth);
            cur = Image::Format::color(red, green, blue, alpha);
          }
          *data++ = cur;
        }
        prevLine = curLine;
  ...
  bool imWritePNG(Image const& image, File& file, bool gray) {
    ...
    Image::Format::color_t const* bits = image.bits();
  ...
  bool imReadPNG(Image& image, File& file) {
    ...
    while (true) {
      ...
      if (ch.type == 0x49454e44 /*IEND*/) {
        ...
        for (uint32 i = 0; i < pal.size; i++) {
          ...
          pal.rgba[i] = Image::Format::color(red, green, blue);
        }
        ...
        if (hdr.colorType == 3) {
          ...
          for (uint32 i = 0; i < pal.size && i < ch.length; i++) {
            pal.rgba[i] = (pal.rgba[i] & ~Image::Format::alpha::mask) | Image::Format::alpha::to(ch.data[i]);
          }
    ...
    if (gzinflate(&buf[0], buf.size(), &src[0], &total_src_size)) {
      return false;
    }
