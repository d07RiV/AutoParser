...
int PowerAttribute() {
  static int attr = fixAttrId(1270, true);
  return attr;
}
    ...
    for (uint32 affixId : recipe->second) {
      ...
      for (auto& attr : affix.attributes) {
        if (attr.type != -1U) {
          ...
          if (attr.type == static_cast<uint32>(PowerAttribute())) {
...
void diff(File& file, json::Value const& lhs, json::Value const& rhs, std::vector<std::string> const& order, bool links) {
  ...
  for (auto& key : keys) {
    ...
    file.printf(">\n");
    if (links) {
      file.printf("   <i><a href=\"/powers/%d/%s\">%s</a></i>\n", SnoLoader::primary->build(), key.c_str(), key.c_str());
    } else {
      file.printf("   <i>%s</i>\n", key.c_str());
    }
    auto subkeys = order;
      ...
      if (lv.type() == json::Value::tArray || rv.type() == json::Value::tArray) {
        ...
        if (!list.empty() && (i < order.size() || firstKeyPtr(list[0].first) != firstKeyPtr(list[0].second))) {
          ...
          for (auto& lr : list) {
            if (sub == "powers") {
              if (lr.second) {
                file.printf("    <a href=\"/powers/%d/%s\">", SnoLoader::primary->build(), firstKey(*lr.second).c_str());
              } else {
                file.printf("    <a href=\"/powers/%d/%s\">", SnoLoader::primary->build(), firstKey(*lr.first).c_str());
              }
            } else {
              file.printf("    ");
            }
  ...
  for (auto& kv : val.getMap()) {
    ...
    for (size_t i = 0; i < subkeys.size(); ++i) {
      ...
      if (lv.type() == json::Value::tArray) {
        ...
        for (auto& lri : lv) {
          if (sub == "powers") {
            file.printf("    <a href=\"/powers/%d/%s\">", SnoLoader::primary->build(), firstKey(lri).c_str());
          } else {
            file.printf("    ");
          }
    ...
    if (kv.first.find("_TagMap") != std::string::npos) {
      for (auto& pv : kv.second.getMap()) {
        ...
        if (dst.type() == json::Value::tArray && pv.first.find(SF) == 0) {
          size_t index = atoi(pv.first.c_str() + SF.length());
          if (index < sfd.length()) {
