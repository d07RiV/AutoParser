...
static uint32 unescape(uint8_const_ptr& chr) {
  ...
  default:
    if (isdigit(*chr)) {
      ...
      return utf8::parse(utf8::transform(&chr, 0));
    }
  }
}
static uint32 getchar(uint8_const_ptr& chr, uint32 table = 0) {
...
void CharacterClass::addClass(CharacterClass const& cls) {
  ...
  if (cls.invert) {
    if (cls.data.size() == 0) {
      ...
      for (size_t i = 1; i < cls.data.size(); i++) {
...
uint8_const_ptr CharacterClass::init(char const* src, int flags) {
  ...
  uint32 table = ((flags & Prog::CaseInsensitive) ? utf8::tf_lower : 0);
...
Prog::Prog(char const* expr, int length, uint32 f) {
  ...
  uint32 ut_table = (flags & CaseInsensitive ? utf8::tf_lower : 0);
...
static bool matcher(Match const& match, void* arg) {
  if (arg) {
    ...
    sub->clear();
    for (size_t i = 0; i < sizeof(match.start) / sizeof(match.start[0]) && match.start[i]; i++) {
