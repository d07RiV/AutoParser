...
void SnoMap::save(std::string const& type) {
  File file(path::work() / fmtstring("sno_%s", SnoLoader::primary->version().c_str()) / type + ".txt", "wt");
  for (const auto& kv : map_) {
    file.printf("%d %s\n", kv.first, kv.second.c_str());
  }
}
bool SnoMap::load(std::string const& type) {
  File file(path::work() / fmtstring("sno_%s", SnoLoader::primary->version().c_str()) / type + ".txt", "rt");
