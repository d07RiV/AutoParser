...
GameTextures::GameTextures() {
  File src(path::work() / fmtstring("sno_%s/TextureMap.dat", SnoLoader::primary->version().c_str()));
  if (!src) {
    ...
    Logger::end();
    File dst(path::work() / fmtstring("sno_%s/TextureMap.dat", SnoLoader::primary->version().c_str()), "wb");
