...
void SkillTips::generate(std::string const& cls, uint32 kitId, bool elems, json::Value* fix) {
  ...
  auto dictSkillsUIEn = Strings::list("SkillsUI", SnoLoader::primary);
  for (auto& kv : catIds) {
  ...
  auto dictPowers = Strings::list("Powers");
  auto dictPowersEn = Strings::list("Powers", SnoLoader::primary);
...
void simBase(std::string const& name) {
  ...
  for (uint32 id : prio) {
    ...
    if (tag.size() == 0 || tag.size() != static_cast<size_t>(tag[0] * 3 + 1)) continue;
